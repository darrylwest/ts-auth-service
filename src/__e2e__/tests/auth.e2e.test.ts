...
describe('Authentication Flow E2E', () => {
  ...
  describe('Token Validation', () => {
    ...
    it('should return 401 when Authorization header is malformed', async () => {
      ...
      const malformedHeaders = ['invalid-format', 'Bearer', 'Bearer ', 'Basic some-token', 'bearer lowercase-bearer'];
    ...
    it('should return 403 when Bearer token is malformed', async () => {
      ...
      const response = await apiClient.withAuth(malformedToken).get('/api/profile');
    ...
    it('should accept valid Firebase token and return user profile', async () => {
      ...
      const response = await apiClient.withAuth(validToken).get('/api/profile');
  ...
  describe('User Profile Creation Flow', () => {
    it('should create new user profile on first authentication', async () => {
      ...
      const response = await apiClient.withAuth(newUserToken).get('/api/profile');
    ...
    it('should use existing profile on subsequent requests', async () => {
      ...
      // First request
      const firstResponse = await apiClient.withAuth(userToken).get('/api/profile');

      // Second request should return the same profile
      const secondResponse = await apiClient.withAuth(userToken).get('/api/profile');
  ...
  describe('Different User Roles Authentication', () => {
    it('should authenticate admin user successfully', async () => {
      ...
      const response = await apiClient.withAuth(adminToken).get('/api/profile');
    ...
    it('should authenticate super-admin user successfully', async () => {
      ...
      const response = await apiClient.withAuth(superAdminToken).get('/api/profile');
